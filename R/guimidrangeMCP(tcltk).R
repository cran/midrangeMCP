...
guimidrangeMCP <- function(gui = TRUE) {
  ...
  if (gui == TRUE) {
    # Salvar o diretorio de trabalho atual
    ...
    tkbind(group_buttons_2, "<ButtonRelease>",
           function(...) {
             ...
             if (length(nchar(as.character(dir_name))) != 0) {
    ...
    tkbind(calculate_button, "<ButtonRelease>", function(...){
      ...
      if (tclvalue(vari3) == gettext("Averages", domain = "R-midrangeMCP")) {
        ...
        # Plotting
        tkplotmidrangeMCP3(frameplot, envmidrangeMCP$results, col = heat.colors(length(levels(trat))))
    ...
    tkpack(plot_button <- ttkbutton(text = gettext("Update plot",
                                    ...
                                    command = function() {
                                      ...
                                      tkplotmidrangeMCP3(frameplot, envmidrangeMCP$results, col = color, horiz = hor, xlab = xlab, ylab = ylab)
    ...
    tkwm.protocol(midrangeMCP, "WM_DELETE_WINDOW", function() {
      ...
      if (as.character(response) == "yes") {
        # Delete objects created in the global environment
